// ============================================================
// Solara Astro Glossary — Phase M2 論点4 (β案 確定)
//
// 占星術専門用語の解説辞書。AstroTermLabel widget と組み合わせて、
// 用語の横にiアイコンを置き、タップでグラスモーフィズム解説を出す。
//
// 設計: project_solara_astrocartography_m2.md 論点4
//   全て専門用語表記 + iアイコンで補助。
// ============================================================

import 'package:flutter/material.dart';

import '../widgets/info_popup.dart';

class AstroGlossaryEntry {
  /// 表示用の正式名 (例: "ASC (上昇宮)")
  final String title;

  /// 1行サマリ (タップ前の hint 表示用)
  final String summary;

  /// 詳細解説 (popup 内)
  final String detail;

  const AstroGlossaryEntry({
    required this.title,
    required this.summary,
    required this.detail,
  });
}

/// 用語キー → 解説。Map widgets から `astroGlossary[key]` で参照。
const Map<String, AstroGlossaryEntry> astroGlossary = {
  // ── 4 アングル ──
  'asc': AstroGlossaryEntry(
    title: 'ASC (Ascendant / 上昇宮)',
    summary: '東の地平線で昇る黄道点。第1ハウスの起点。',
    detail:
        '生まれた瞬間に東の地平線で昇っていた黄道上の点。第1ハウスのカスプ (起点) で、'
        '外面的な人格・第一印象・身体的特徴を司る。\n\n'
        'リロケーションで地点を変えると ASC の星座が変わり、'
        'その土地で「どんな人として現れるか」が変化する。',
  ),
  'mc': AstroGlossaryEntry(
    title: 'MC (Midheaven / 天頂)',
    summary: '子午線上の黄道点。第10ハウスの起点。',
    detail:
        '生まれた瞬間に天頂 (子午線と黄道の交点) にあった点。第10ハウスのカスプで、'
        '社会的役割・キャリア・公的な評価を司る。\n\n'
        'リロケーションで MC の星座が変わると、その土地での「社会的位置づけ」'
        'が変化する。「ハリウッド MC」のように特定の都市で MC を活かすという考え方も。',
  ),
  'dsc': AstroGlossaryEntry(
    title: 'DSC (Descendant / 下降宮)',
    summary: '西の地平線へ沈む黄道点。第7ハウスの起点。',
    detail:
        'ASC の真反対に位置する点。第7ハウスのカスプで、'
        'パートナーシップ・対人関係・結婚を司る。\n\n'
        'ASC が「自分」なら DSC は「自分にとっての他者」。'
        'リロケーションで DSC が変わると出会う相手の傾向が変わる。',
  ),
  'ic': AstroGlossaryEntry(
    title: 'IC (Imum Coeli / 天底)',
    summary: '子午線下の黄道点。第4ハウスの起点。',
    detail:
        'MC の真反対に位置する点。第4ハウスのカスプで、'
        '家庭・ルーツ・心の拠り所を司る。\n\n'
        'リロケーションで IC が変わると、その土地での「家としての落ち着き」'
        'の感覚が変わる。',
  ),

  // ── ハウス (12個) ──
  'house_1': AstroGlossaryEntry(
    title: '第1ハウス (アセンダント・ハウス)',
    summary: '自我・身体・第一印象。',
    detail: '自分自身、身体、性格の表れ方。ASC が起点。'
        'ここに惑星があるとその性質が「外に出る」。',
  ),
  'house_2': AstroGlossaryEntry(
    title: '第2ハウス (お金のハウス)',
    summary: '所有・金銭・自己価値。',
    detail: '稼ぐ力、所有物、自分の価値観。物質的な安定と自己肯定感の源。',
  ),
  'house_3': AstroGlossaryEntry(
    title: '第3ハウス (コミュニケーション)',
    summary: '会話・学び・近距離移動・兄弟姉妹。',
    detail: '日常的な会話、学習、近所、兄弟姉妹、短距離旅行。情報処理の場。',
  ),
  'house_4': AstroGlossaryEntry(
    title: '第4ハウス (家庭のハウス)',
    summary: '家・ルーツ・心の拠り所。IC が起点。',
    detail: '家、家族、出身、心の安全基地。終末期、晩年も司る。',
  ),
  'house_5': AstroGlossaryEntry(
    title: '第5ハウス (創造のハウス)',
    summary: '恋愛・遊び・創造性・子供。',
    detail: 'ロマンス、芸術表現、ギャンブル、子育て。喜びを生み出す場。',
  ),
  'house_6': AstroGlossaryEntry(
    title: '第6ハウス (仕事と健康)',
    summary: '日常業務・健康・奉仕。',
    detail: '日々のルーティンワーク、体調管理、奉仕。職人的な技術習得。',
  ),
  'house_7': AstroGlossaryEntry(
    title: '第7ハウス (パートナーシップ)',
    summary: '結婚・対人・契約。DSC が起点。',
    detail: '結婚相手、ビジネスパートナー、公然の敵。1対1の対峙関係。',
  ),
  'house_8': AstroGlossaryEntry(
    title: '第8ハウス (深淵のハウス)',
    summary: '他者の資源・性・変容・遺産。',
    detail: 'パートナーの財産、性、深い結びつき、遺産、心理的変容、生死。',
  ),
  'house_9': AstroGlossaryEntry(
    title: '第9ハウス (高次の探求)',
    summary: '哲学・宗教・遠距離旅行・高等教育。',
    detail: '海外、思想、大学、宗教、長距離移動。視野を広げる場。',
  ),
  'house_10': AstroGlossaryEntry(
    title: '第10ハウス (天職のハウス)',
    summary: '社会的役割・キャリア・名声。MC が起点。',
    detail: '職業、社会的地位、評判、目標。世間からの見え方。',
  ),
  'house_11': AstroGlossaryEntry(
    title: '第11ハウス (友愛のハウス)',
    summary: '友人・サークル・理想・未来計画。',
    detail: '友人、コミュニティ、希望、長期計画、社会改革。',
  ),
  'house_12': AstroGlossaryEntry(
    title: '第12ハウス (秘密のハウス)',
    summary: '無意識・秘密・隠遁・霊性。',
    detail: '無意識、隠された敵、入院、出家、霊的探求。表に出ないもの。',
  ),

  // ── Phase M2 機能用語 ──
  'relocation': AstroGlossaryEntry(
    title: 'リロケーション (Relocation Chart)',
    summary: '出生時刻はそのまま、別の土地でハウスを再計算。',
    detail:
        '惑星の位置 (黄経) は出生時刻で確定し、地点を変えても変わらない。'
        'しかし ASC・MC・12ハウスのカスプは「観測者の位置と方角」で決まるため、'
        '別の土地に立つと再計算される。\n\n'
        '結果: 同じ太陽でも「東京では5H」「ハワイでは10H」のように、'
        '体験するハウスが変わる = その土地での自分の出方が変わる。',
  ),
  'relocate_layer': AstroGlossaryEntry(
    title: '引越しレイヤー〔Pro〕',
    summary: 'タップ地点を引越し先に見立て、動く星のライン・ASC/MC・ハウスを比較。',
    detail:
        'Solara の引越し検討ツール (Cosmic Pro 限定)。地図タップで、'
        'その土地に住んだ場合の変化をまとめて表示する:\n\n'
        '・現住所と比べて近づく / 遠ざかる星のライン (動く星のライン)\n'
        '・ASC / MC の星座変化\n'
        '・10惑星の 12 ハウス再配置\n\n'
        '比較ベースは現住所 (未設定なら出生地)。'
        '個人天体 (太陽・月・水星・金星・火星) のハウスが大きく変わる土地は、'
        '人生の方向性が変わる可能性のある土地と言える。',
  ),
  'aspect_lines': AstroGlossaryEntry(
    title: 'アスペクト線 (Astro*Carto*Graphy / Natal)',
    summary: '出生時の各惑星×ASC/MC/DSC/ICの世界地図上ライン。',
    detail:
        'Jim Lewis (1970年代) が体系化したアストロカートグラフィの主要手法。'
        '出生時の10惑星 × 4アングル = 40本のラインを地球曲面に投影。'
        '一生変わらない「本質の地図」。\n\n'
        '線上の土地ではその惑星のエネルギーが特定のアングルで強く効く。\n'
        '・金星 ASC ライン → 関係性・調和のエネルギーが顔として現れる\n'
        '・木星 MC ライン → 拡大と機会のエネルギーが社会的役割に乗る\n'
        '・土星 IC ライン → 構造化と継続のエネルギーが家庭の根に効く\n\n'
        'これらは「良い」「悪い」ではなく、その土地に在るエネルギーの性質。'
        '読み取って、自分でどう向き合うかを決める。',
  ),
  'transit_acg': AstroGlossaryEntry(
    title: 'Transit線 (Cyclo*Carto*Graphy)',
    summary: '今この瞬間の天体位置で引いた40本のライン。毎日動く。',
    detail:
        'Jim Lewis が A*C*G の続編として 1982 年に体系化した CCG '
        '(Cyclo*Carto*Graphy)。'
        '出生時ではなく「今 (またはタイムスライダーで指定した時刻)」'
        'の天体位置を世界地図に投影する。\n\n'
        '線は時間と共に動く。地球の自転で MC/IC は1日360°、'
        'ASC/DSC は緯度依存で蛇行しながら西へ流れる。\n\n'
        '使い方:\n'
        '・木星 ASC が今どこを通っているか → 拡大のエネルギーが集中する地点\n'
        '・土星 MC が来週東京を通る → 構造化と選択のタイミング\n\n'
        '線が示すのはエネルギーの所在。「ラッキー」「アンラッキー」ではなく、'
        '在るエネルギーをあなた自身がどう読み、どう向き合うかが本体。',
  ),
  'progressed_acg': AstroGlossaryEntry(
    title: 'Progressed線 (Secondary Progression)',
    summary: '2次進行 (1日=1年) の天体位置で引いた40本。',
    detail:
        '2次進行 (1日=1年) で進めた天体位置を A*C*G ライン化したもの。'
        '人生の長期テーマがどこに現れるかの地図。\n\n'
        '出生から30年経った人なら、出生から30日後の天体位置を使う。'
        '太陽は約1°/年でゆっくり動き、月は約12°/年。\n\n'
        '「現在の自分の本質」が活性化する土地を示す。'
        'Natal線 (出生固定) より動きが緩やかで、Transit線より深い意味を持つ。',
  ),
  'solar_arc_acg': AstroGlossaryEntry(
    title: 'Solar Arc線 (ソーラーアーク方向)',
    summary: '太陽の進行弧を全惑星に等しく加算した位置でのライン。',
    detail:
        'ソーラーアーク・ディレクション = 太陽の2次進行による移動弧 '
        '(arc = 進行太陽 - 出生太陽) を全惑星に同じだけ加算する古典的予測法。\n\n'
        'Progressed と異なり、すべての惑星が同じ速度 (太陽速度) で動くため、'
        '惑星間のアスペクト構造が崩れず、人生の重要転機の'
        'タイミングを示す指標として伝統的に重視される。\n\n'
        'CCG では Solar Arc 木星MC 通過 = 達成と拡大の流れが集まる年、'
        'Solar Arc 土星ASC = 責任と構造の年、のように読む。'
        '「成功」「失敗」ではなく、その期間に在るエネルギーの性質を示す。',
  ),
  // ── B1: アスペクト線 (square / trine / sextile) ──
  // 本線 (conjunction) が惑星とアングルの重なりなのに対し、惑星がアングルから
  // 90°/120°/60° 離れた地点を結ぶライン。本線より細い破線で描かれる。
  'aspect_lines_full': AstroGlossaryEntry(
    title: 'アスペクト線 (120 本拡張)',
    summary: 'コンジャンクション 40 本に、スクエア / トライン / セクスタイル 80 本を加えた全 120 本表示。',
    detail:
        'デフォルトの ACG ライン (本線) は惑星とアングルが重なる '
        'コンジャンクション 40 本 (10 惑星 × 4 アングル) のみ。\n\n'
        'このトグルを ON にすると、各惑星から:\n'
        '・スクエア (90°) — Hard エネルギー\n'
        '・トライン (120°) — Soft エネルギー\n'
        '・セクスタイル (60°) — Soft エネルギー (穏やか)\n'
        'の 3 種類の追加ラインが引かれ、本線 40 + 拡張 80 = 計 120 本になる。\n\n'
        'このトグルは単一フラグで、現在 ON 中の全フレームに同時に適用される。'
        'Natal / Transit / Prog / S.Arc のうち線 ON 中のフレームすべてで、'
        'それぞれ 80 本ずつ拡張線が引かれる (フレーム別に独立 ON/OFF はできない)。\n\n'
        '拡張線は本線より細い破線で、色は惑星色を Soft/Hard エネルギー色へ '
        '寄せて描画される (吉凶色 = 赤緑にはしない)。\n\n'
        '4 フレーム (Natal / Transit / Prog / S.Arc) 全部 ON だと最大 480 本に '
        'なるため、第 3 層の FORTUNE カテゴリで惑星を絞ると見やすい。\n\n'
        'Solara 設計思想: Soft / Hard は独立した 2 つのエネルギー。'
        '吉凶ではなく、その土地に在るエネルギーの性質として読む。',
  ),
  'aspect_square': AstroGlossaryEntry(
    title: 'スクエア線 (90°)',
    summary: '惑星がアングルから90°離れた地点のライン。Hard エネルギー。',
    detail:
        '惑星とそのアングルが90°を結ぶ地点を通る線。'
        '本線 (コンジャンクション) より細い破線で描かれる。\n\n'
        'スクエアは Hard エネルギー — 摩擦や緊張をともなう。'
        'ただしそれは「悪い」のではなく、向き合うことで自分の知らない面に出会い、'
        '関係や仕事を深く築き直す機会になる。覚悟が要るかもしれない。'
        '受け止められるかは、あなた次第。\n\n'
        'Solara は Soft/Hard を独立した2つのエネルギーとして扱う。'
        '吉凶ではなく、その土地に在るエネルギーの性質を読み取る。',
  ),
  'aspect_trine': AstroGlossaryEntry(
    title: 'トライン線 (120°)',
    summary: '惑星がアングルから120°離れた地点のライン。Soft エネルギー。',
    detail:
        '惑星とそのアングルが120°を結ぶ地点を通る線。'
        '本線より細い破線で描かれる。\n\n'
        'トラインは Soft エネルギー — 流れがなめらかで、才能や調和が'
        '自然に発揮されやすい。努力なしに進む心地よさがある一方、'
        '安定しすぎて変化が起きにくいこともある。\n\n'
        'Solara は Soft/Hard を独立した2つのエネルギーとして扱う。'
        '吉凶ではなく、その土地に在るエネルギーの性質を読み取る。',
  ),
  'aspect_sextile': AstroGlossaryEntry(
    title: 'セクスタイル線 (60°)',
    summary: '惑星がアングルから60°離れた地点のライン。Soft エネルギー。',
    detail:
        '惑星とそのアングルが60°を結ぶ地点を通る線。'
        '本線より細い破線で描かれる。\n\n'
        'セクスタイルは Soft エネルギー — トラインより穏やかで「機会」の性質を持つ。'
        '自分から一歩動けば扉が開く、誘いのようなエネルギー。'
        '動かなければ静かなまま。\n\n'
        'Solara は Soft/Hard を独立した2つのエネルギーとして扱う。'
        '吉凶ではなく、その土地に在るエネルギーの性質を読み取る。',
  ),
  'zenith_point': AstroGlossaryEntry(
    title: '天頂点 (Zenith Point)',
    summary: '惑星が真上(高度90°)に来る、地球上で唯一の点。',
    detail:
        'MC ライン上で 緯度=惑星の赤緯δ となる唯一の地点。'
        'その地点に立つと惑星は文字通り「真上」にある (高度90°)。\n\n'
        'Jim Lewis は天頂点について「単なる1点ではなく、その緯度線全周に効く」'
        'と説いた。つまり同じ緯度上の都市は経度を問わず、その惑星のエネルギーを'
        '受ける、というのが Lewis 流の読み方 (緯度効果)。\n\n'
        'MC ラインの帯状の影響に対し、天頂点は「最も垂直に降り注ぐ」スポット。'
        '吉凶ではなく、エネルギーがその地に在ること、それをどう受け取るかが本体。',
  ),
  'nadir_point': AstroGlossaryEntry(
    title: '天底点 (Nadir Point)',
    summary: '惑星が真下(地球の裏側)にある、天頂点の対称地点。',
    detail:
        'IC ライン上で 緯度=-δ となる地点。'
        '天頂点と地球中心を挟んで完全に対称、観測者から見ると惑星は'
        '足元の真下 (大地の裏側) にある。\n\n'
        'Lewis 理論では、天底もその緯度線全周に効くとされる (天頂と同じ緯度効果)。'
        '天頂が「社会的露出・キャリア・名声」に効くなら、'
        '天底は「家庭・ルーツ・無意識・基盤」に効く磁場。\n\n'
        'IC ラインの帯状の影響に対し、天底点は「最も深く根に降りる」スポット。'
        '家系・記憶・内面に静かに、しかし深く働きかける性質を持つ。',
  ),
  'altitude_event': AstroGlossaryEntry(
    title: '高度と天頂寄り / 天底寄り',
    summary: 'MC/IC 通過時の惑星の高度。90° に近いほど真上から / 真下から効く。',
    detail:
        'Daily Transit の MC (天頂通過) / IC (天底通過) イベント時、その瞬間の'
        '惑星の高度を表示する。Lewis 流の見方に直結:\n\n'
        '・MC 通過 高度 ≈ 90° → 観測者緯度がほぼ惑星赤緯 = 「真上から降る」'
        '日。年に数回しか起こらない強い瞬間。\n'
        '・IC 通過 高度 ≈ -90° → 観測者の足下を惑星が通る = 「内側から動く」'
        '日。天頂寄り日とペアで現れる。\n\n'
        '★マーク (天頂寄り / 天底寄り) は閾値 ±85° (ACG オーブと整合)。'
        '通常の MC 通過は高度 30〜70° 程度で、これは「強さは普通だが方向は天頂」'
        'を意味する。',
  ),
  'latitude_band_now': AstroGlossaryEntry(
    title: '今あなたの緯度帯 (Latitude Band Now)',
    summary: '観測時刻に、あなたと同じ緯度線上で天頂/天底を迎えている惑星。',
    detail:
        'Lewis 流の緯度効果 — 「天頂帯/天底帯は同じ緯度線全周に効く」 — を'
        '時間軸に持ち込んだもの。\n\n'
        '惑星の赤緯 (δ) が観測者の緯度に近いとき (オーブ ±5°):\n'
        '・δ ≈ 観測者緯度 → 緯度線上のどこかで惑星天頂、その緯度全周にエネルギー\n'
        '・δ ≈ -観測者緯度 → 緯度線上のどこかで惑星天底、その緯度全周に深層エネルギー\n\n'
        '観測者は自分の経度がたとえ「真上点」から離れていても、Lewis 流では'
        '同じ緯度線上に居るだけでその惑星のエネルギーを受け取るとされる。'
        'これは ACG の Latitude Band 概念を Daily Transit に投影したセクション。',
  ),
  'latitude_band': AstroGlossaryEntry(
    title: '天頂帯 (Zenith Latitude Band)',
    summary: '天頂・天底を貫く緯度線、同じ緯度全周に効くという Lewis 流。',
    detail:
        'Jim Lewis は「天頂点はそのポイントだけでなく、同じ緯度線全周に惑星エネルギーを'
        '発動させる」と説いた。経度を問わず、北緯δの都市はすべて天頂効果を、'
        '南緯-δの都市はすべて天底効果を受ける、という見立て (緯度効果)。\n\n'
        '視覚的には、各惑星の天頂・天底から東西方向に伸びる帯として描画される。'
        '天頂帯 (実線): 社会的露出・キャリアに効く緯度群。\n'
        '天底帯 (点線): 家庭・ルーツ・無意識に効く緯度群。\n\n'
        '情報密度が高いため、デフォルトは OFF。Lewis 流の読み方を試したい時に ON にする。',
  ),
  'sector_score_16': AstroGlossaryEntry(
    title: '16方位エネルギー',
    summary: '各方角に在る2つの独立したエネルギー（ソフト・ハード）を表示。',
    detail:
        'natal/transit/progressed の総合アスペクトを16方位 (N, NNE, NE...) に投影し、'
        '方角ごとに「ソフトエネルギー」「ハードエネルギー」を独立して算出する。\n\n'
        '☯ ソフト: 流れに乗る力（受容・拡大・調和・安定）\n'
        '☐ ハード: 摩擦と変容の力（対峙・成長・見直し・選択）\n\n'
        '2つは1つの軸の両端ではなく、それぞれ独立した別のエネルギー。'
        '両方が同時に強く効くこともあれば、片方だけのこともある。\n\n'
        'この情報は、方角を選んで動く時、行かざるを得ない方角に向かう時、'
        '向こうから訪れる出来事と向き合う時、いずれにも使える。'
        'Solara は「ここが良い」「ここが悪い」とは言わない。'
        '在るエネルギーを伝える。読み取り、向き合うのはあなた。\n\n'
        '引越しレイヤーとは別系統 (ローカルスペース流派)。'
        '同時表示時は引越しレイヤーを主としてこちらは dim 表示する。',
  ),
  'planet_lines': AstroGlossaryEntry(
    title: '惑星方位ライン (ローカルスペース)',
    summary: '出生地起点に各惑星の方角線を放射状に描画。',
    detail:
        'ローカルスペース流派の手法。出生地を中心とした「方位の家」を描き、'
        '各惑星がどの方向にあるかを線で示す。\n\n'
        'アストロカートグラフィ (アスペクト線) とは流派が異なる。'
        '前者は「ローカル方位」、後者は「世界曲線」。',
  ),
  'placidus': AstroGlossaryEntry(
    title: 'Placidus (プラシーダス)',
    summary: '時間ベースで12ハウスを分割する伝統的方式。',
    detail:
        'プラチドス・デ・ティティス (17世紀) のハウスシステム。'
        '太陽が ASC から MC まで動く時間を3等分してハウス境界を決める。\n\n'
        '高緯度 (|lat|>66°) では計算破綻するため、Solara は自動的に Equal House に'
        'フォールバックする。',
  ),
  // ── FORTUNE カテゴリ (CategoryPills の i アイコン用、2026-04-29) ──
  'fortune_all': AstroGlossaryEntry(
    title: '総合 (All Categories)',
    summary: '全10惑星のラインを100%表示。フィルタなし。',
    detail:
        '占星術の全領域を一望するモード。10惑星 × 4アングル = 40本の'
        'アスペクト線をすべて同じ強度で表示する。\n\n'
        '個別の運勢カテゴリではなく、「人生全体のエネルギー地図」'
        'として土地の総合的な性質を確認したいときに使う。',
  ),
  'fortune_love': AstroGlossaryEntry(
    title: '恋愛 (Love)',
    summary: '金星・火星・月のラインを強調、他は dim。',
    detail:
        '関係性・愛のテーマに関わる惑星のみを際立たせるフィルタ。\n\n'
        '・金星 (Venus): 愛・美・関係性\n'
        '・火星 (Mars): 情熱・行動・欲求\n'
        '・月 (Moon): 感情・親密さ\n\n'
        'これら3惑星のラインが集まる土地・時刻は、'
        '関係性のエネルギーが多面的に効く。'
        'ソフト面とハード面の両方が現れることがあり、'
        '流れに乗るのも、摩擦と向き合うのも、両方を体験するのも、選択はあなた。',
  ),
  'fortune_money': AstroGlossaryEntry(
    title: '豊かさ (Abundance)',
    summary: '木星・金星・太陽のラインを強調、他は dim。',
    detail:
        '物質的豊かさ・価値の創出に関わる惑星のフィルタ。\n\n'
        '・木星 (Jupiter): 拡大・機会・寛容\n'
        '・金星 (Venus): 価値・所有・美\n'
        '・太陽 (Sun): 自己実現・公的役割\n\n'
        'これらの ASC/MC ラインが通る土地・時刻は、'
        '豊かさのエネルギーが流れやすい。'
        '同時にハードな見直しのエネルギーが効くこともある。'
        'どちらも在るものとして読み取る。',
  ),
  'fortune_work': AstroGlossaryEntry(
    title: '仕事 (Career)',
    summary: '土星・火星・木星・太陽のラインを強調、他は dim。',
    detail:
        'キャリア・社会的役割に関わる惑星のフィルタ。\n\n'
        '・土星 (Saturn): 責任・規律・構造化\n'
        '・火星 (Mars): 推進力・突破\n'
        '・木星 (Jupiter): 機会・拡大\n'
        '・太陽 (Sun): リーダーシップ・公的役割\n\n'
        '特に MC ライン (天頂) が重要 — その土地での「社会的な顔」を示す。'
        'ソフト寄りの土地は流れに乗る年、ハード寄りの土地は基盤を作り直す年として読める。',
  ),
  'fortune_communication': AstroGlossaryEntry(
    title: '話す (Communication)',
    summary: '水星・金星・月のラインを強調、他は dim。',
    detail:
        'コミュニケーション・知的活動・対話に関わる惑星のフィルタ。\n\n'
        '・水星 (Mercury): 思考・言語・情報伝達\n'
        '・金星 (Venus): 社交・調和・魅力\n'
        '・月 (Moon): 共感・感情伝達\n\n'
        'これらのラインが通る土地は、執筆・対話・教育・発信のエネルギーが効く。'
        'ハード寄りの場合は誤解や論争のエネルギーも併存することがある。',
  ),
  'fortune_healing': AstroGlossaryEntry(
    title: '癒し (Healing)',
    summary: '月・海王星・金星のラインを強調、他は dim。',
    detail:
        '休息・統合・内省に関わる惑星のフィルタ。\n\n'
        '・月 (Moon): 安らぎ・無意識・母性\n'
        '・海王星 (Neptune): 直感・夢・統合\n'
        '・金星 (Venus): 美・喜び・自己受容\n\n'
        'これらのラインが通る土地は、リトリート・療養・内省のエネルギーが流れる。'
        'ハード成分が強い時は、向き合うべき内的なテーマが浮上することもある。',
  ),

  // ── Daily Transit (CCG)「今日の TOP」選定ロジック（2026-04-29 追加） ──
  'top_category_logic': AstroGlossaryEntry(
    title: '「今日の TOP」の選び方',
    summary: '5カテゴリ × 担当惑星 × アスペクトの総量が最大のものを TOP として表示。',
    detail:
        'Solara はその日の各カテゴリのエネルギー総量を比較し、'
        '最も多く動いているカテゴリを TOP に出します。\n'
        '「ラッキー」「アンラッキー」ではなく、'
        '「エネルギーの動きが最大」という意味です。\n\n'
        '─────────────\n'
        '計算の流れ\n'
        '─────────────\n'
        '① 各カテゴリに担当惑星セットが定義されている\n'
        '② トランジット惑星 × natal / progressed 惑星のアスペクトを検出\n'
        '③ 担当惑星同士のペアアスペクト = 2倍、片側のみ担当 = 0.5倍\n'
        '④ アスペクトの性質（soft / hard / neutral）を Solara 設計思想の'
        '独立2軸に振り分けて積算\n'
        '⑤ 各惑星の方位から16方位へ cos² 減衰で拡散\n'
        '⑥ 16方位 × カテゴリで合算 → カテゴリ合計\n'
        '⑦ 合計が最大のカテゴリが TOP\n\n'
        '─────────────\n'
        '🌿 癒し (healing)\n'
        '─────────────\n'
        '担当惑星: 月・海王星・金星\n'
        'ペア2倍: 月×海王星 / 月×金星 / 太陽×海王星\n'
        '核: 月（情緒の流れ）と海王星（統合・夢想）のペア\n\n'
        '─────────────\n'
        '💰 豊かさ (money)\n'
        '─────────────\n'
        '担当惑星: 木星・金星・太陽\n'
        'ペア2倍: 木星×金星 / 木星×太陽 / 金星×太陽\n'
        '核: 木星（拡大）と金星（価値）のペア\n\n'
        '─────────────\n'
        '💕 恋愛 (love)\n'
        '─────────────\n'
        '担当惑星: 金星・火星・月\n'
        'ペア2倍: 金星×火星 / 金星×月 / 火星×月\n'
        '核: 金星×火星（関係性の動き）— 古典占星術の核\n\n'
        '─────────────\n'
        '💼 仕事 (work)\n'
        '─────────────\n'
        '担当惑星: 土星・太陽・火星・木星\n'
        'ペア2倍: 土星×太陽 / 土星×火星 / 木星×太陽 / 木星×火星\n'
        '核: 土星（責任・構造化）と社会的天体のペア\n\n'
        '─────────────\n'
        '💬 対話 (communication)\n'
        '─────────────\n'
        '担当惑星: 水星・太陽・金星・月\n'
        'ペア2倍: 水星×太陽 / 水星×金星 / 水星×月\n'
        '核: 水星（思考・言語）が必ず一方に入る\n\n'
        '─────────────\n'
        '注意\n'
        '─────────────\n'
        '・「TOP = 良い」ではなく「TOP = 動きが多い」\n'
        '・ソフト・ハード両方の量を含むため、TOP の日は'
        '両エネルギーが効きやすいだけ\n'
        '・ハード成分が多い日は、見つめ直しや内省の機会が多い日',
  ),

  // ── Daily Transit (CCG) 4アングルの占星術的意味（2026-04-29 追加） ──
  'transit_angles': AstroGlossaryEntry(
    title: '4アングル通過の意味',
    summary: '惑星があなたの拠点で ASC/MC/DSC/IC を通過する瞬間の解釈。',
    detail:
        '惑星が地平線・子午線にかかる瞬間は、その惑星のテーマが'
        '特定の形で活性化する時刻です。\n\n'
        '🌅 ASC（昇り始める）\n'
        'その惑星のテーマが新しく芽生える瞬間。\n'
        '東の地平に現れる、外向きの始まり。\n\n'
        '☀ MC（天頂）\n'
        'その惑星のテーマが最も顕在化する瞬間。\n'
        '社会的・公的な力が頂点に達する。\n\n'
        '🌇 DSC（沈む）\n'
        'その惑星のテーマが内向きに転じる瞬間。\n'
        '西の地平に降る、対人関係の文脈に入る。\n\n'
        '🌑 IC（地下）\n'
        'その惑星のテーマが無意識・内的領域で働く瞬間。\n'
        '地下を通る、内省・ルーツの文脈で効く。\n\n'
        '※「良い時刻」「悪い時刻」ではなく、'
        'エネルギーがどこに集まる瞬間かの提示です。'
        'どう向き合うかはあなた自身が決めます。',
  ),

  // ── Solara 設計思想キーワード（2026-04-29 追加、2026-05-11 中立アスペクト追記） ──
  'two_energies': AstroGlossaryEntry(
    title: '2つの独立したエネルギー（ソフト・ハード）',
    summary: 'ソフトとハードは1軸の両端ではなく、それぞれ別のエネルギー。',
    detail:
        'Solara はアスペクトのエネルギーを「ソフト」と「ハード」の'
        '2つの独立した軸として扱う。\n\n'
        '☯ ソフトエネルギー — 流れに乗る力\n'
        '寛容・拡大・受容・調和・安定。物事が滑らかに進み、'
        '出会いが生まれ、心が開かれるエネルギー。\n'
        '元になるアスペクト: トライン (120°)、セクスタイル (60°)。\n\n'
        '☐ ハードエネルギー — 摩擦と変化の力\n'
        '挑戦・変容・対峙・成長。葛藤の中で見えてくるものがあり、'
        '深い学びと、自分自身との出会いが起こるエネルギー。\n'
        '元になるアスペクト: スクエア (90°)、オポジション (180°)、'
        'クインカンクス (150°)。\n\n'
        '○ 中立アスペクト — 両方を兼ねる\n'
        'コンジャンクション (合, 0°) は2つの惑星が同じ位置で重なる関係。'
        'どちらか一方ではなく、ソフトとハードの両方の性質を同時に帯びる。\n'
        'スコア計算では weight を半分ずつ (50:50) ソフト・ハード両方の'
        '量に加算する。両エネルギーが同時に効く場として現れる。\n\n'
        '両方とも、それぞれが美しいエネルギー。'
        'プラスマイナスではなく、性質の違い。'
        '両方が同時に強く効くこともある — そのとき深い体験が起こる。',
  ),
  'soft_aspect': AstroGlossaryEntry(
    title: 'ソフトアスペクト',
    summary: 'トライン (120°)・セクスタイル (60°) 等、流れを生むアスペクト。',
    detail:
        '惑星間が 60°・120° の調和的角度をなすアスペクト。\n\n'
        'ソフトエネルギーを生む。流れ・受容・調和・拡大・統合。'
        '物事が滑らかに進み、新しい出会いや機会が現れやすい。\n\n'
        'ただし「良い」のではなく「流れやすい」だけ。'
        '流れにそのまま乗るか、意識的に方向を選ぶかは、あなたの選択。',
  ),
  'hard_aspect': AstroGlossaryEntry(
    title: 'ハードアスペクト',
    summary: 'スクエア (90°)・オポジション (180°) 等、摩擦と変容を生むアスペクト。',
    detail:
        '惑星間が 90°・180° の緊張的角度をなすアスペクト。\n\n'
        'ハードエネルギーを生む。摩擦・対峙・選択・変容・成長。'
        '違和感や葛藤を通して、見えていなかったものが見えてくる。\n\n'
        '「悪い」のではない。むしろ深い学びが起こる時間。'
        '見つめ直すか、行動するか、距離を取るかは、あなたが選ぶ。',
  ),

  // ── Daily Transit カテゴリ別「おすすめ行動の例」の意図（2026-04-30 追加） ──
  'category_tips_intent': AstroGlossaryEntry(
    title: 'おすすめ行動の例 — 使い方',
    summary: 'これは「指示」ではなく「ヒント」。あなた自身で動きを考える参考に。',
    detail:
        'このフィルタで見える時間帯は、選んだカテゴリのエネルギーが\n'
        '動く瞬間です。表示されている行動例は「絶対にこうすべき」\n'
        'という指示ではありません。\n\n'
        '─────────────\n'
        '行動例の意図\n'
        '─────────────\n'
        '・このカテゴリ × アングル相（外向き／内向き）の典型的な\n'
        '　動き方を例示する\n'
        '・あなた独自の状況に合わせて、動きを応用するヒントとして使う\n'
        '・「自分ならどう動くか」を考えるきっかけにする\n\n'
        '─────────────\n'
        '応用の例\n'
        '─────────────\n'
        '例えば「新しい出会いの場や対話のきっかけを意識する」とあれば、\n'
        '実際の動きはあなたの生活に合わせて自由です。\n'
        '・友人を誘って食事に行く\n'
        '・SNSで久しぶりの知人に連絡する\n'
        '・職場で隣の席の人にひとこと声をかける\n'
        '・お店で店員さんと少し話してみる\n'
        '・新しいコミュニティを覗いてみる\n'
        'いずれも「対話のきっかけを意識する」の応用です。\n\n'
        '─────────────\n'
        'Solara の姿勢\n'
        '─────────────\n'
        'Solara はエネルギーがどこに動いているかを伝えるだけ。\n'
        '「ラッキー」「アンラッキー」とは言いません。\n'
        'どう関わるか、何をするかは、あなたが選びます。',
  ),
};

/// 用語解説 popup を表示する共通ヘルパー。
/// LayerPanel / FramePills / CategoryPills 等から共通で呼ぶ。
/// [termKey] が astroGlossary に存在しなければ何もしない。
///
/// 2026-05-07: 統一 popup ヘルパー [showInfoPopup] 経由に移行。
/// 右上 × / 全文スクロール / 外タップ閉じが Shell 側で自動提供される。
void showAstroGlossaryDialog(BuildContext context, String termKey) {
  final entry = astroGlossary[termKey];
  if (entry == null) return;
  showInfoPopup(
    context: context,
    maxWidth: 360,
    child: Column(
      mainAxisSize: MainAxisSize.min,
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Text(entry.title,
            style: const TextStyle(
              fontSize: 14,
              color: Color(0xFFC9A84C),
              fontWeight: FontWeight.w600,
              letterSpacing: 0.4,
            )),
        const SizedBox(height: 8),
        Text(entry.summary,
            style: const TextStyle(
              fontSize: 11,
              color: Color(0xFFAAAAAA),
              height: 1.5,
              letterSpacing: 0.3,
            )),
        const Divider(color: Color(0x22FFFFFF), height: 18),
        Text(entry.detail,
            style: const TextStyle(
              fontSize: 12,
              color: Color(0xFFE8E0D0),
              height: 1.7,
              letterSpacing: 0.2,
            )),
      ],
    ),
  );
}
