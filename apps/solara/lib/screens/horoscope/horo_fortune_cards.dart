...
class HoroFortuneCards extends StatelessWidget {
  final int fortuneIdx;
  final PageController pageCtrl;
  final ValueChanged<int> onPageChanged;

  const HoroFortuneCards({
    super.key,
    required this.fortuneIdx,
  ...
  Widget build(BuildContext context) {
    return Column(children: [
      ...
      SizedBox(height: 180, child: PageView.builder(
        controller: pageCtrl, itemCount: fortuneCategories.length,
        onPageChanged: onPageChanged,
        itemBuilder: (ctx, i) {
          final cat = fortuneCategories[i];
          ...
          return Container(
            ...
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Row(children: [
                ...
                const Spacer(),
                // HTML: .fortune-card-score { gradient text }
                ShaderMask(
      ...
      Row(mainAxisAlignment: MainAxisAlignment.center, children:
        List.generate(fortuneCategories.length, (i) => Container(
          width: i == fortuneIdx ? 20 : 8, height: 8,
          margin: const EdgeInsets.symmetric(horizontal: 3),
          decoration: BoxDecoration(
            borderRadius: BorderRadius.circular(i == fortuneIdx ? 4 : 4),
            color: i == fortuneIdx ? const Color(0xFFF6BD60) : const Color(0x26FFFFFF),
            boxShadow: i == fortuneIdx
      ...
      Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
        _fortuneNavBtn('← 前へ', () {
          if (fortuneIdx > 0) {
            pageCtrl.previousPage(duration: const Duration(milliseconds: 350), curve: Curves.easeOut);
          }
        }),
        _fortuneNavBtn('次へ →', () {
          if (fortuneIdx < fortuneCategories.length - 1) {
            pageCtrl.nextPage(duration: const Duration(milliseconds: 350), curve: Curves.easeOut);
          }
        }),
      ]),
    ]);
