...
class MoneyPainterBuilder extends FortunePainterBuilder {
  ...
  static const _pieceCount = 260;
  static const _dustCount = 160;
  static const _sparkleCount = 40;
  static const _numColumns = 20; // 積み上げ用のカラム数
  ...
  List<_GoldPiece> _buildPieces() {
    const palettes = <_GoldPalette>[
      // Antique gold (くすんだ古金貨)
      _GoldPalette(
        highlight: Color(0xFFF5DFA8), main: Color(0xFFC9A24A),
        shadow: Color(0xFF4A3010), backside: Color(0xFF8A6820),
        edge: Color(0xFFE8CC7A), rim: Color(0xFFE0BC6A),
      ),
      // Sepia gold (羊皮紙ゴールド)
      _GoldPalette(
        highlight: Color(0xFFEED1A0), main: Color(0xFFB08238),
        shadow: Color(0xFF3A2408), backside: Color(0xFF6C4C18),
        edge: Color(0xFFD8B468), rim: Color(0xFFCCA858),
      ),
      // Patina (古美術の緑青がかった金)
      _GoldPalette(
        highlight: Color(0xFFD8E0B8), main: Color(0xFF9A9050),
        shadow: Color(0xFF2E2E14), backside: Color(0xFF645C2E),
        edge: Color(0xFFBAB888), rim: Color(0xFFAFAB7A),
      ),
      // Bronze / brass
      _GoldPalette(
        highlight: Color(0xFFE8C8A0), main: Color(0xFFB07840),
        shadow: Color(0xFF3E1E08), backside: Color(0xFF6E4822),
        edge: Color(0xFFCF9C66), rim: Color(0xFFC68E58),
      ),
      // Pale antique (少量、象牙金)
      _GoldPalette(
        highlight: Color(0xFFF8EACA), main: Color(0xFFE0C078),
        shadow: Color(0xFF4A3818), backside: Color(0xFFA88850),
        edge: Color(0xFFEFD4A0), rim: Color(0xFFE5C890),
      ),
      // Tarnished silver (少量の銀・希少性)
      _GoldPalette(
        highlight: Color(0xFFE0E0DC), main: Color(0xFFA0A098),
        shadow: Color(0xFF303038), backside: Color(0xFF60605C),
        edge: Color(0xFFBCBCB4), rim: Color(0xFFB0B0A8),
      ),
    ];
    // 刻印シンボル: 太陽・月・惑星・星・ローマ数字・錬金術。
    // Variation Selector-15 (\uFE0E) で絵文字グリフへの置換を抑止し、
    // serif 活字でアンティークに見せる。
    ...
    for (var i = 0; i < _pieceCount; i++) {
      ...
      final isCoin = _rng.nextDouble() < 0.55; // 55% コイン, 45% 金箔/羊皮紙片
      // 銀はコインの5%にのみ
      var paletteIdx = _rng.nextInt(palettes.length - 1);
      if (_rng.nextDouble() < 0.05) paletteIdx = palettes.length - 1;
      pieces.add(_GoldPiece(
        ...
        delay: math.pow(_rng.nextDouble(), 0.85).toDouble() * 0.62,
        fallDuration: 0.22 + _rng.nextDouble() * 0.18,
...
class _MoneyPainter extends CustomPainter {
  ...
  static const _layerStep = 0.033;
  // 床のベースY（画面高さ比）
  static const _floorY = 0.95;

  @override
  void paint(Canvas canvas, Size size) {
    final diag = math.sqrt(size.width * size.width + size.height * size.height);

    // 背景: 羊皮紙セピア → 深焦茶のクラシカルグラデ
    ...
    if (bgAlpha > 0) {
      canvas.drawRect(Offset.zero & size, Paint()
        ..shader = ui.Gradient.linear(
          ...
          [
            Color.fromRGBO(238, 210, 164, 0.40 * bgAlpha), // 羊皮紙クリームセピア
            Color.fromRGBO(158, 100, 50, 0.28 * bgAlpha),  // 焦げ茶
            Color.fromRGBO(60, 30, 12, 0.22 * bgAlpha),
      ...
      canvas.drawRect(
        ...
        Paint()
          ..shader = ui.Gradient.radial(
            ...
            size.width * 0.75,
            [
              Color.fromRGBO(230, 180, 90, 0.50 * pileGlow * bgAlpha),
              Color.fromRGBO(180, 120, 50, 0.26 * pileGlow * bgAlpha),
              const Color(0x00000000),
            ],
            [0.0, 0.45, 1.0],
          )
  ...
  void _drawCoin(Canvas canvas, _GoldPiece p, double faceVis, bool isBack, double alpha) {
    ...
    if (!isBack) {
      ...
      // 刻印シンボル（錬金術/占星術記号）— serif+VS-15 でアンティーク活字
      if (s >= 22) {
        final symbolFontSize = s * 0.55;
        final engravePaint = Paint()
          ..color = pal.shadow.withValues(alpha: 0.78 * alpha);
        final tp = TextPainter(
          text: TextSpan(
            text: p.symbol,
            style: TextStyle(
              fontSize: symbolFontSize,
              foreground: engravePaint,
              height: 1.0,
              fontFamily: 'serif',
        ...
        // 刻印のハイライト（わずかに上ずらし、光り方を演出）
        final highlightPaint = Paint()
          ..color = pal.highlight.withValues(alpha: 0.45 * alpha);
        final tpHl = TextPainter(
          text: TextSpan(
            text: p.symbol,
            style: TextStyle(
              fontSize: symbolFontSize,
              foreground: highlightPaint,
              height: 1.0,
              fontFamily: 'serif',
              fontFamilyFallback: const ['Noto Serif', 'Times New Roman', 'DejaVu Serif'],
            ),
        ...
        tpHl.layout();
        tpHl.paint(canvas, Offset(-tpHl.width / 2 - s * 0.012, -tpHl.height / 2 - s * 0.012));
      }
    }
