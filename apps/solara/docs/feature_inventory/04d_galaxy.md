...
- L48 `createState()`
- L123 `regenerateBackground()` — タブ切替でGalaxyに入ってきた時に、背景 (ネビュラ位置・色・星の位置)
- L134 `pauseMotion()` — main.dart から Galaxy タブ離脱時に呼ばれる。Timer 即停止 = raster 0% 化。
- L154 `initState()`
- L165 `jitter()`
- L183 `dispose()`
- L383 `build()`
