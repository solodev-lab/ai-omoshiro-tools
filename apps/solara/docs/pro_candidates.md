...
> 🔴 **2026-05-31 Pro→Free 巻き戻し (オーナー指示)**: 下記 5 機能 (いずれもコスト 0 =
> クライアント計算) を **Free に戻した**。各表のゲート状態セルに `↩ Free (2026-05-31)` を併記。
> 1. **ACG Transit / Progressed / SolarArc フレーム** (§7.2) — natal と合わせ 4 フレーム全て Free。
>    `aspectLines` (120 本) と `relocate` (引越し) は **Pro 維持**。
> 2. **形成演出を再生** (§7.3 C5) — Star Atlas カード長押しメニュー。
> 3. **テキストとしてコピー** (§7.3 C5)。
> 4. **Galaxy Archive 検索・フィルタ** (§7.3 C2) / **タロット履歴 検索・フィルタ** (§7.3 C3)。
> 5. **Stella 相談結果のシェア** (§7.2 (ii) Stage 4)。
>
> **Pro 維持** (今回戻さなかったもの): 柱1 全部 (A1/A2/A3) / アスペクト 120 本 / 引越し /
> 時計スライダー 1 分刻み / VP・LOCATION 枠 / Forecast 期間 / 称号再診断 (C4)。
...
- **シェアボタン**：**v1 から UI 設置**（画像/テキスト export）。↩ **Free (2026-05-31、オーナー指示で Pro ゲート撤廃)**
...
| 層 | 項目 | ゲート状態 |
|---|---|---|
| 看板（ローンチ） | 称号ギャラリー（C4、変遷の可視化、**Pro 維持**）/ Galaxy アーカイブ（C2、閲覧ビュー） | C4=✅ 柱3-A / C2 検索フィルタ=↩ Free (2026-05-31) |
| 同梱（ローンチ） | タロット履歴の検索・フィルタ（C3）/ 形成演出の再生 + エクスポート（C5） | ↩ Free (2026-05-31、旧 柱3-A で Pro 化 → 巻き戻し) |
...
- ~~**消費型（クレジット/都度課金）併設** — §5 論点#9~~ → **✅ 決定・実装済 (2026-05-23)**。
  「1 クレジット = AI 占い 1 回」を Stella 相談 + タロットカテゴリで共通の財布として実装。
  Free=無料週次3回 (端末ID・月曜UTCリセット) → 購入残高 (消費型IAP・サインイン必須) → Pro=無制限。
  Free は Pro 同等品質 (thinking ON)。設計詳細は機能インベントリ §0.2.1 +
  メモリ `project_solara_stella_free_credits.md`。残=ストア商品作成 + deploy + 実機検証。
