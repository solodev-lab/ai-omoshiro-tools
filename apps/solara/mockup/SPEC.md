...
##### 再診断ルール (Flutter版 確定仕様 / 2026-05-13)

| 条件 | 動作 |
|------|------|
| 初回診断 | Reveal は「これでいく」のみ。redoCount は 0 のまま保存 |
| 再診断 (Free 1 回まで) | Sanctuary 画面の「再診断する」ボタンから実行。Reveal で「これでいく」または「✦ 前のクラスと比較する」を選択。どちらでも redoCount=1 として保存 |
| 上限到達後 (Free) | 「再診断はCosmic Pro限定」ラベルでボタン無効化 |
| Cosmic Pro (将来) | 無制限やり直し + **コレクション機能** (過去の全クラス保存・閲覧・切替)。差別化軸は「回数」ではなく「記録性」 |

**ロジック詳細:**
- court 判定 / wildcard 補正 / 同点解消はすべて **占星術シード** (太陽×月星座、144通り) で公平化
- 順序バイアス (旧 page/power 優先) を完全排除
- 「もう一度診断する」(初回 Reveal) は廃止 (Free 上限の抜け道を防ぐ)
- 比較 popup: 前のクラスの ClassCard + light/shadow を表示、「前のクラスを採用する」or「新しいまま」を選択
